...
class CommentProjectController extends Controller
{
    ...
    public function index(){
        ...
        return view('administrator.comment_project.index');
    }
    
    public function getData(Request $request){
        $data = CommentProject::query()
                                ->with('project')
                                ->where('comment_id',0);
        ...
        return DataTables::of($data)
            ->addColumn('action', function ($row) {
                ...
                if (isAllowed(static::$module, "detail")) : //Check permission
                    $btn .= '<a href="'.route('admin.comment_project.detail',$row->id).'" data-id="' . $row->id . '" class="btn btn-secondary btn-sm mx-1">
                    Detail
                </a>';
                endif;
                return $btn;
    ...
    public function delete(Request $request)
    {
        ...
        $data = CommentProject::with('reply')->findOrFail($id);
        if (!$data) {
            abort(404);
        }
    ...
    public function detail($id){
        ...
        $data = CommentProject::where('id', $id)->first();
        if (!$data) {
            abort(404);
        }

        return view('administrator.comment_project.detail',compact('data'));
    }

    public function getDataDetail(Request $request, $id){
        $data = CommentProjectReply::query()
                                ->with('project')
                                ->where('comment_id', $id);
    ...
    public function deleteDetail(Request $request)
    {
        ...
        // Find the data based on the provided ID or throw a 404 exception.
        $data = CommentProjectReply::findOrFail($id);
        if (!$data) {
            abort(404);
        }
        // Store the data to be logged before deletion
