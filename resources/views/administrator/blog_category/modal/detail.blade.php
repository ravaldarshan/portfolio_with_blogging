<!-- Modal Detail CategoryProject -->
<div class="modal fade" tabindex="-1" role="dialog" id="detailCategoryBlog" data-backdrop="false">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="detailCategoryBlogLabel">Detail Category Project</h5>
                <button type="button" class="close" id="buttonCloseModuleModal" data-dismiss="modal"
                    aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" id="detailCategoryBlogBody">
...
@push('js')
    <script>
        $('#detailCategoryBlog').on('show.bs.modal', function(event) {
            var button = $(event.relatedTarget);
            var id = button.data('id');

            var modalBody = $('#detailCategoryBlogBody');
            ...
            $.ajax({
                url: '{{ route('admin.category_blog.getDetail', ':id') }}'.replace(':id', id),
                method: 'GET',
                    ...
                    if (data.blog != '') {
                        ...
                        for (let i = 0; i < data.blog.length; i++) {
                            ...
                            modalBody.append(
                                ...
                                '</div>' +
                                '<div class="col-7">' +
                                '<div class="data">: ' + data_blog.description + '</div>' +
                                '</div>' +
                                '</div>' +
                                '<br>'
                            );
