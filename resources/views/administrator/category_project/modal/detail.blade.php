...
<div class="modal fade" tabindex="-1" role="dialog" id="detailCategoryProject" data-backdrop="false">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="detailCategoryProjectLabel">Detail Category Project</h5>
                <button type="button" class="close" id="buttonCloseModuleModal" data-dismiss="modal"
                    aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" id="detailCategoryProjectBody">
...
@push('js')
    <script>
        $('#detailCategoryProject').on('show.bs.modal', function(event) {
            var button = $(event.relatedTarget);
            var id = button.data('id');

            var modalBody = $('#detailCategoryProjectBody');
                    ...
                    if (data.project) {
                        ...
                        for (let i = 0; i < data.project.length; i++) {
                            ...
                            modalBody.append(
                                ...
                                '</div>' +
                                '<div class="col-7">' +
                                '<div class="data">: ' + data_project.description + '</div>' +
                                '</div>' +
                                '</div>' +
                                '<br>'
                            );
